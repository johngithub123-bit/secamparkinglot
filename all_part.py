...
while 1:
		...
		if cursor.rowcount == 0:
			...
			if status == 1:
				...
				if Inside < MaxParked:
					...
					query = "INSERT INTO log (CheckTime, VehicleID, CheckType, ParkingLot, VerificationType) VALUES (\'" + CheckTime + "\', \'" + vehicle_name + "\', 0, \'P7\', 1);"
					...
					displayMsg("YOUR PALECE IS FULL TAKE TICKET OR PUNCH CARD.")
		...
		if len(x0) != 0:
			...
			if cursor.rowcount != 1:
				...
				if status == 1:
					...
					if Inside < MaxParked:
						print("You can park your car")
						displayMsg("You can park your car")
						break
					else:
						print("The parkinglot for this company is alreay full.")
						displayMsg("YOUR PALECE IS FULL TAKE TICKET.")
	...
	if GPIO.input(BUTTON) == 0:
		...
		fob.write('^FB400,5,,\n')
		fob.write('^FD --------------\&\n')
		fob.write('PARKING P7\&\n')
		str1 = 'DATE: %d. %s. %d \&\n'%(today.day, today.strftime('%B')[0:3], today.year)
		fob.write(str1)
		str1 = ' TIME: %02d:%02d:%02d\&\n'%(today.hour, today.minute, today.second)
		fob.write(str1)
		# Print Barcode
		str1 = 'Car Plate: %s^FS\n'%(lprcode)
		fob.write(str1)
		...
		str1 = "^FD%02d%02d%02d%02d%02d%02d%1d^FS\n"%(today.year%100, today.month, today.day, today.hour, today.minute, today.second, 7)
		barcode = "%02d%02d%02d%02d%02d%02d%1d"%(today.year%100, today.month, today.day, today.hour, today.minute, today.second, 7)
		fob.write(str1)
		fob.write('\n')

		fob.write('^CF0,30,25^FO25,550\n')
		fob.write('^FB520,4,,\n')
		fob.write('^FD Emergency Phone:+381 11 234234^FS\n')
		fob.write('^XZ\n')
		fob.close()

		os.system("lpr -P Zebra_Technologies_ZTC_KR403 -o raw raw.zpl")
