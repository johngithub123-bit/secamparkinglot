...
while 1:
	...
	if GPIO.input(Loop0) == 1:
		...
		displayMsg("")
		time.sleep(0.02)
		displayMsg("Ulazna rampa nije zatvorena!")
		print("car is loop0")		
		# GPIO.output(Barrier1, GPIO.LOW)		# Barrier1 is open
		# GPIO.output(Barrier2, GPIO.LOW)			# Barrier2 is close
	...
	while GPIO.input(Loop0) == 1:
		time.sleep(0.02)
		displayMsg("Rampe nisu zatvorene!")
		...
		if cursor.rowcount == 0:
			...
			if status == 0:
				...
				if cursor.rowcount != 1:
						...
						displayMsg("Kartica nije aktivna za ovaj parking! Ocitajte drugu karticu.")
						continue
				MaxParked = row[1]
				Inside = row[2]
				companyId = row[3]
				...
				displayMsg("Tablica ocitana, prodjite")
	...
	displayMsg("Prinesite karticu ili ocitajte TIKET")
	...
	while 1:
		...
		if len(x0) != 0:
			...
			if cursor.rowcount != 1:
				...
				displayMsg("Kartica ne postoji ocitajte tiket")
				# GPIO.output(DENYLED, GPIO.HIGH)
				# relay_status = relay_status | DENYLED
				# bus.write_byte_data(I2C_ADDRESS, 0x09, relay_status)
				# time.sleep(0.3)
				...
				if status == 0:
					print("This car is already out. Please try another card or get the ticket from the service.")		# go to the button for thermal printer
					displayMsg("Vozilo je van parkinga. Pokusajte sa drugom karticom ili ocitajte TIKET")
					# GPIO.output(DENYLED, GPIO.HIGH)
					# relay_status = relay_status | DENYLED
					# bus.write_byte_data(I2C_ADDRESS, 0x09, relay_status)
					# time.sleep(1)
					...
					while GPIO.input(Loop0) == 1:
						time.sleep(0.02)
						displayMsg("Izlazna rampa je otvorena")
						print("barriers are open. loop0 = 1.")	
						#GPIO.output(barrier1, GPIO.LOW)
						relay_status = relay_status | (Barrier2)
						bus.write_byte_data(I2C_ADDRESS, 0x09, relay_status)
		...
		if len(barcode) != 0:
			...
			if cursor.rowcount != 1:
				...
				displayMsg("Nepostojeci TIKET! Ocitajte drugi TIKET!")
				# GPIO.output(Barrier1, GPIO.HIGH)
				# relay_status = relay_status | Barrier1
				# bus.write_byte_data(I2C_ADDRESS, 0x09, relay_status)
				continue
			...
			if PaidOrNot == 0:
				...
				if StatusTicket_pay == 0 or StatusCar_in == 0 or IfCanLeave == 0:
					...
					if StatusTicket_pay == 0:
						displayMsg("Tiket nije placen!")
					elif IfCanLeave == 0:
						displayMsg("Prekoracili ste vreme za izlazak!")
					else:
						displayMsg("Tiket je vec ponisten!")
					# GPIO.output(Barrier1, GPIO.HIGH)
					# relay_status = relay_status | Barrier1
					# bus.write_byte_data(I2C_ADDRESS, 0x09, relay_status)
